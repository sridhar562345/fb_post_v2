...
from fb_post_v2.build.responses.GetPostsIdsResponse.GetPostsIdsResponse.GetPostsIdsResponseSerializer import GetPostsIdsResponseSerializer
...
app_name = "fb_post_v2"
operation_id  = "posts_with_more_postive_recations"
group_name = ""

@request_response(options=options, app_name=app_name, operation_id=operation_id, group_name=group_name)
def posts_with_more_postive_recations(request, *args, **kwargs):
    args = (request,) + args
    from django_swagger_utils.drf_server.wrappers.view_env_wrapper import view_env_wrapper
    return view_env_wrapper(app_name, "posts_with_more_postive_recations", group_name, *args, **kwargs)
